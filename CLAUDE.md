...
2026-08-05: Pan Yao inquiry Q1 on the value of selectivity (detection frequency + PNEC risk of analogue competitors) adopted as a binding project-wide criterion. Added `docs/SELECTIVITY_VALUE_CRITERION_2026-08-05.md` (norm + branch audit + per-scheme actions), framework §十二, SPEC closure criterion 10, and appended audit addenda (append-only) to the six passed scheme files on this branch: S05 (largely compliant), S14 (non-compliant, rectify with main BPA20-01), S17 (compliant, positive example), S26 (compliant, positive example), S27 (partially non-compliant), S32 (partially non-compliant). Committed by Claude Code at Pan Yao's direct instruction (cross-branch alignment round covering main/Qwen/Ultimate/dsreview).
