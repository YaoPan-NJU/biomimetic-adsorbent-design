...
> **2026-07-19科学分类与课题范围裁决：仿生与离子交换不是互斥类别，静电吸引也不等于离子交换。但PFH-1的PS-DVB主体、叔胺外层和季铵内核使其必须优先归类为“生物转运启发的分区型阴离子交换树脂”；严格仿生因果尚未成立。该材料与本项目需要避开的离子交换树脂方向重合，因此PFH-1终止，M0不再执行。本报告保留为设计与否决审计。先读`docs/SCIENTIFIC_CONSENSUS_BIOMIMETIC_AND_ION_EXCHANGE.md`，再读项目边界和Gate G30。**
...
原报告曾建议先执行`PFH1_M0_PROTOCOL.md`，该建议已被Gate G29撤回。PFH-1属于离子交换树脂方向，不再进行制造或PFAS吸附验证。当前动作是重新检索具有真实生物吸附/富集因果证据、且能够转译为非离子交换主机制的人工材料。

本报告现在作为失败路线审计：它说明了空间分层不能改变材料由离子交换主导的根本类别，也防止后续设计再次用仿生语言包装交换树脂。
