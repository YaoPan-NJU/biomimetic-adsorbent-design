...
## 20 条方案逐项六维专家审查检查点（2026-07-31）

已在前述全分支去重审查之上，对 20 条设计线逐一回答六个问题：化学上是否成立、工程上是否可用、现路线能否得到目标材料、真实水体是否可能有效去除、再生是否有工程价值、修改后潜力与停止条件。完整报告：`deliverables/attachments/DETAILED_PER_SCHEME_EXPERT_REVIEW_2026-07-31.md`。

### 处置没有被旧分数改变

- **A 级优化主线：** SC-P02 / PG-PMO；BPA20-01 / AG-COF；DDT50-01 / LPO-SIP。
- **B 级生死门：** GX50-01；DCP20-01；NP A01；PFHxS R2_S03；Dieldrin A01。
- **C 级研究假设/对照：** PFOA A02；PFBS A17；ROX A01；Octocrylene A01/A04；BPA PXR；PCP A04；beta-HCH A08/A12。
- **D 级当前淘汰：** BDE-209 A07；Ultimate DDT A01；Endosulfan A01（作为吸附剂）。

### 新增的根因级发现

1. **Octocrylene A04 有酸碱计算硬错误。** 原稿采用 `pKa=10.2` 却把 pH 7 的哌嗪游离碱写成约 99%；若该 pKa 对应反应性 N 的共轭酸，一元模型正确值约 0.063%。N-甲基哌嗪的严格位点分布仍需微观 pKa 或直接速率验证，但原稿的 99% 结论确定不成立。EDC/NHS 路线还会消耗唯一二级胺，随机共接枝也不能保证 thiourea 位于 3–5 Å 邻位。该路线移出吸附材料验证。
2. **PCP A04 的目标物种与精准腔叙事错位。** PCP `pKa≈4.5–4.7`，pH 7 时几乎全为 pentachlorophenolate，不是原稿假定的中性酚；已有 beta-CD 聚合物实验还显示 PCP 性能可低于 GAC。该方案只保留为 conventional beta-CD/GAC 对照。
3. **多条承重 DOI 错配。** PCP A04 的两个 DOI 分别指向透明质酸药物递送水凝胶和 BPA/BPS 斑马鱼研究；beta-HCH A12 的两个 Nature DOI 分别指向抗体论文更正和 Galectin-3/HSC 研究。相关合成、再生和笼腔证据不能继续使用。
4. **“反应能发生”不等于“得到目标空间结构”。** SC-P02 的延迟共缩合、DDT/GX/DCP 的延迟核壳聚合、Dieldrin 的随机共接枝、COF/PAF 的后修饰均必须用截面/深度化学映射和同结构对照证明，不能由投料配方代替。
5. **再生必须闭合污染物去向。** 高比例醇、盐或碱把 PFAS/OCP 转移到浓缩废液不等于完成处置；后续统一报告质量回收、废液体积、材料溶出、压力损失和至少 10 周期寿命。

### 负责人审批后的唯一允许动作

1. SC-P02：四材料同批 QC，先证明非离子交换的空间梯度。
2. BPA20-01：先做可溶锚存在/删除模型，水相锚增量不过门则停止 COF。
3. DDT50-01：先完成分析系统和三相质量平衡，再做七材料同批 M0。
...
## 20 条方案仿生—材料—合成链人工裁决底稿（2026-07-31）

已将第二轮权威处置中的 20 条设计逐一还原为负责人可直接人工判别的完整链条，文件为 `deliverables/attachments/HUMAN_ADJUDICATION_DOSSIER_20_SCHEMES_2026-07-31.md`。每条均包含：

1. 真实生物/界面原型及其实际功能；
2. 原型机理中的事实、可转译推断和未经验证假设；
3. 原型特征到人工材料特征的对应及必要因果对照；
4. 目标材料实体和源稿合成路线；
5. 现路线能否形成声称的孔口、薄壳、双层、腔体或人工识别单元；
6. 第三方修改路线、停止条件和负责人裁决问题。

底稿维持当前权威处置 A3/B2/C12/D3，且不把缺失 exact monomer、连接位、计量或空间结构控制的概念路线补写成虚假的可执行 SOP。新增的关键判断包括：beta-HCH A12 把阴离子 Cl- 稳定穴外推到中性共价 C-Cl，属于承重化学映射错误；Octocrylene A04 必须先证明高度拥挤烯烃的分子反应动力学；Endosulfan A01 若继续，应改立介孔 Lewis 酸催化水解课题而非吸附剂；BDE-209 A07 和 Ultimate DDT A01 的现有材料反应/位点设计不能通过文本修补获得准入。

### 负责人现在需要做的事

按底稿第 4 节的七个问题，对 20 条逐项给出 `是 / 否 / 需补证据`。人工裁决完成前不采购、不合成、不进入完整性能验证。若批准 Gate 0，只按以下顺序推进：

1. SC-P02：先承认径向外层梯度，不预称孔口；冻结 5 mmol 设备级 SOP 和残胺/CTAB/IEC 假阳性门。
2. DDT50-01：放弃同锅延迟投料定义薄壳，以预制种珠后的表面引发或经验证两段种子聚合重做路线。
3. BPA20-01：先做可溶锚存在/删除/门控模型，再决定是否冻结一个确切 COF 家族。
4. GX50-01、DCP20-01：只允许一票否决的 pre-M0，不按现稿直接投料。

C 类只可拆为离散人工识别单元或模型反应基础课题；D 类旧版本停止。所有方案仍未获准进入完整 E1 或动态床 E2。

---

## 污染物结构—吸附去除机制扩展检查点（2026-07-31）

负责人指出上一版虽已覆盖仿生原型、映射、材料和合成，但没有先讲清“针对什么污染物、针对什么结构特征”。已直接扩展同一权威底稿 `deliverables/attachments/HUMAN_ADJUDICATION_DOSSIER_20_SCHEMES_2026-07-31.md`，不另造相互竞争的报告版本。

本次补充包括：

1. 把 20 条设计还原为 15 类真实靶标，给出结构简式、近中性水中主要形态、普通材料基线去除机制和方案声称的选择性增量；
2. 明确技术 4-NP 是支化异构体混合物，HCH/endosulfan 必须区分立体异构体，PFAS 必须同时区分头基和链长；
3. 为全部 20 条方案逐一增加“污染物结构与吸附/去除机制”卡，说明结构把手、普通材料为何也会去除、以及必须出现的差分因果签名；
4. 新增静电/离子交换、疏水分配/色散、氢键/酸碱、形状互补、CT 和反应性去除六类机制的判别表；
5. 把负责人裁决清单由原七问扩展为九问，先问靶标身份、水中形态与结构把手，再审仿生、材料、合成、真实基质和再生/产物闭合。

结构身份来源已登记到 `research/evidence/evidence_ledger.csv` 的 E111–E125；这些记录只支持分子身份和官能团地图，不把数据库结构记录升级为水相结合、吸附性能或风险证据。

### 对处置的影响

A3/B2/C12/D3 数量不变，但理由更明确：

- SC-P02 必须证明头基夹持与短尾域分区的空间协同，而不是残胺/正电 + 链长分配；
- DDT50-01 必须把普通疏水/颗粒分配与 C12 相态窗口、印迹增量分开；
- BPA20-01 必须出现锚删除与桥区门控对 BPA/BPF/BPS/BPAF 的不同影响；
- GX50-01 极易退化为普通阴离子交换；DCP20-01 的酸碱形态和甲醚假模板均与声称的酚 OH 锚错位；
- PCP A04 在近中性水中主要面对酚盐，beta-HCH A12 将共价 C-Cl 错当 Cl-，BDE-209 醚 O 被多溴位阻遮蔽，三条原结构叙事均不能按现稿推进；
- Octocrylene A04、Ultimate DDT A01 和 Endosulfan A01 若继续，必须按反应/转化项目做全产物质量平衡，不能沿用可逆吸附—再生话语。

### 当前唯一下一动作

负责人按扩展底稿第 4 节九问逐条人工判别；人工判别前仍不采购、不合成、不进入完整性能验证。Gate 0 顺序和权限边界不变：SC-P02、DDT50-01、BPA20-01 仅做各自承重机理的最小验证；GX/DCP 只做一票否决 pre-M0；C/D 不按旧高分推进。

---

## 按项目科学主线重写方案叙事（2026-07-31）

负责人指出上一版虽然信息完整，但污染物结构卡、仿生映射卡、合成卡和审稿意见彼此割裂，读起来仍像字段汇总，不像一个可以人工判断的完整科研方案。已依据 `SOUL.md`、`docs/BIOMIMETIC_DESIGN_FRAMEWORK.md`、`rounds/fresh_1000/GLOBAL_SPEC.md` 和项目现有深度研究文稿，直接重写同一权威底稿 `deliverables/attachments/HUMAN_ADJUDICATION_DOSSIER_20_SCHEMES_2026-07-31.md`。

每条方案现在沿同一条因果链展开：

> 污染物难去除的根因 → 传统材料缺少的能力 → 生物原型的真实功能 → 可转译的设计原理 → 人工材料与合成路线 → 预期选择性去除过程 → 可证伪实验 → 第三方处置

这次修改不是删掉污染物结构和吸附机制，而是把它们放回方案开头，先说明靶标在水中的真实形态、普通材料为何也会去除，再解释仿生材料需要贡献什么独立增量。A/B 类保留完整方案叙述、合成可行性诊断、修改路线和负责人裁决问题；C/D 类用连续短论证说明为什么只值得做离散模型、为什么应停止，或为什么应改立反应性处理课题。

### 本次没有改变的边界

- 权威处置仍为 A3/B2/C12/D3；没有方案因此升入 E1。
- 没有新增材料性能、结合自由能或工程寿命主张。
- `research/evidence/evidence_ledger.csv` 的 E111–E125 仍只支持结构身份和官能团地图。
- exact material、空间结构、合成SOP或再生废流未闭合的地方继续明确写为缺口，没有用顺畅叙事掩盖证据不足。

### 当前唯一下一动作

负责人直接阅读因果叙事版底稿，优先裁决 SC-P02、DDT50-01、BPA20-01 的科学问题和路线重建范围，再决定 GX50-01、DCP20-01 是否值得进入一票否决 pre-M0。人工裁决前不采购、不合成、不进入完整验证；C/D 不得按旧分数推进。

---

## 负责人首轮逐条裁决与核壳架构偏置修正（2026-07-31）

负责人完成了SC-P02之后的首批人工判别。决定已登记到 `deliverables/attachments/USER_REVIEW_LOG.md` 的U-017至U-022，并写回 `deliverables/attachments/HUMAN_ADJUDICATION_DOSSIER_20_SCHEMES_2026-07-31.md`。负责人决定控制执行权限，优先于此前第三方A/B/C/D预分类。

### 已明确的处置

- **SC-P02：** 化学分类上不是传统树脂，但负责人不接受微米级径向基团梯度作为强仿生硬对应。等待分子尺度识别单元重建；若只保留梯度PMO，应移出仿生主方案。
- **DDT50-01：** 当前版本停止。原St/DVB核壳合成可操作性和目标薄壳成功率不可接受，不再用更复杂的表面引发聚合修补旧路线。
- **BPA20-01：** 保留，下一轮优先详细讨论；当前不授权COF合成。
- **GX50-01：** 当前版本停止。理想化核壳制造和对完整GenX缺乏说服力的生物原型同时构成停止理由。
- **DCP20-01：** 保留2,6-DCP科学问题，材料架构重做。停止把从头构筑St/DVB核壳作为默认答案，转为高比表面积载体表面功能化/表面印迹筛选。
- **PFBS A17、ROX A01、PCP A04：** 保留讨论，但未批准材料合成或M0。

### 核壳偏置的根因判断

此前A/B五条中有三条采用St/DVB核加功能薄层。该重复度无法由DDT、GenX和2,6-DCP三种不同污染物的科学问题共同解释，说明核壳被当成了共享设计模板。后续材料选择必须遵守载体中立原则：只有当从头构筑的核本身承担不可替代的识别、相态或工程功能时，才允许承担悬浮聚合、珠径控制和壳层定位风险。

DCP的替代路线不直接锁定活性炭。颗粒活性炭具有装柱成熟和高比表面积优势，但其氯酚本底吸附可能掩盖印迹增量，薄层聚合也可能堵塞微孔。Gate 0设计必须比较颗粒活性炭、层级孔碳和一种低本底无机载体，并设置未修饰载体、偶联/引发载体、NIP、MIP、亲水层和MIP加亲水层。判据是DOM中保留的MIP-NIP选择性增量、孔容/传质损失、批间重复性、总复合材料质量利用率和床层性能，不能只比较空白水总容量。

### 当前唯一下一动作

下一轮先讨论BPA20-01，顺序为污染物类似物判别问题、DmpR/PoxR原型证据、分子锚、桥区门、可溶模型及COF准入条件；随后依次讨论PFBS A17、ROX A01和PCP A04。DCP只设计载体筛选门，不采购、不投料。DDT50-01和GX50-01当前版本停止，所有保留讨论方案仍未获准进入M0/E1。
