...
面向市政二级出水选择性吸附的独立研究仓库。`main` 分支由 Codex 维护并同步 `origin/main`，`kimi-k3` 分支由 Claude Code 维护并同步 `origin/kimi-k3`；两条分支独立演进，互不交叉推送。Pan Yao 负责科学决策，实验交接对象为 Zhou Jiaqi。本项目与 ADRMATS 无关。

## 当前结论

当前没有通过的主方案或备选方案，`primary_design` 与 `backup_design` 均为 `null`。BPA、ROX、PFOA、PFBS 和 2,6-DCP 五污染物的 100 方案扩展（各 20 个）已完成淘汰与全局排序，见 [`rounds/portfolio_100/RANKING.md`](rounds/portfolio_100/RANKING.md)。无方案达到 85 通过线。

**框架修正（2026-07-18）**：Pan Yao 识别出项目执行中存在思路偏移——许多方案采用"找到能结合污染物的蛋白"（思路三），而项目主干路线应为"解决工程难题（思路二）+ 机制匹配（思路一）"。完整的仿生设计框架修正见 [`docs/BIOMIMETIC_DESIGN_FRAMEWORK.md`](docs/BIOMIMETIC_DESIGN_FRAMEWORK.md)。下一步是用修正后的框架重新评估 portfolio_100 方案。
...
新会话入口：`kimi-k3` 分支从 [`CLAUDE.md`](CLAUDE.md) 起，`main` 分支从 [`AGENTS.md`](AGENTS.md) 起；两者规则一致。随后按以下顺序阅读：
...
6. [`docs/BIOMIMETIC_DESIGN_FRAMEWORK.md`](docs/BIOMIMETIC_DESIGN_FRAMEWORK.md) — **仿生设计框架（2026-07-18 修正版）**
7. [`docs/source/仿生吸附材料设计任务总纲.md`](docs/source/仿生吸附材料设计任务总纲.md)

`PROJECT_STATE.yaml` 是机器可读状态，`HANDOFF.md` 保存完整跨设备交接和历史解释，`BIOMIMETIC_DESIGN_FRAMEWORK.md` 是所有子 Agent 必须遵循的设计框架。
...
### 第六阶段：五污染物 100 方案与框架修正（2026-07-18）

1. 扩展至五污染物（BPA、ROX、PFOA、PFBS、2,6-DCP），各生成 20 个机制可区分方案，共 100 方案。经各污染物轮淘汰至各 5 条保留方案后，完成全局排序与讨论集切片，见 [`rounds/portfolio_100/RANKING.md`](rounds/portfolio_100/RANKING.md)。无方案达 85 通过线。
2. **框架修正**：Pan Yao 识别出项目执行中存在思路偏移——许多方案采用"找到能结合污染物的蛋白"（思路三），而项目主干路线应为"解决工程难题（思路二）+ 机制匹配（思路一）"。完整的仿生设计框架修正见 [`docs/BIOMIMETIC_DESIGN_FRAMEWORK.md`](docs/BIOMIMETIC_DESIGN_FRAMEWORK.md)。
3. **下一步**：用修正后的框架重新评估 portfolio_100 方案，检查每个方案是否：解决了传统方法解决不了的工程难题、具有至少 2 个硬对应维度、生物原型真实且相关、材料复杂度可控、痛点从仿生能力倒推。
4. **术语标准**：统一使用"人工识别单元"（artificial recognition unit）指代人工合成的分子识别结构，不使用"受体"（receptor）以避免与生物蛋白受体混淆。
