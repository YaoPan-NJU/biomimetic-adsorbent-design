...
潘尧随后明确希望挽救该方向，并要求按最高质量重构。新报告 [`MAC-IF1挽救式重构`](DEEP_RESEARCH_ROX_MAC_IF1_RESCUE_DESIGN.md) 把主场景原型改为成熟失活污水生物膜的CLA/ERY/ROX共同富集，把阴离子磷脂界面作为空间机制支持，并提出`MAC-IF1`：在毫米St–DVB多孔球可达孔壁上形成低覆盖度、短程羧酸浅层，保留紧邻的低极性孔面。均匀`MAC-BI1`没有删除，转为`UNIFORM`因果对照。

最新工程边界见 [`G27`](orchestration/gates/G27_ROX_SOLID_LOW_COMPLEXITY_BOUNDARY.yaml)：最终形态只接受可回收颗粒或固定床吸附剂。生物膜或阴离子脂质界面只提供设计依据，不意味着制作膜分离材料或加入天然膜成分。优先采用商品/简单悬浮聚合多孔球加一次低成本后处理，或直接成球加水解；UV接枝降为工艺对照，不再锁定为默认路线。马弗炉处理可以进入候选，但必须保留可检验的仿生功能，不能仅得到普通炭材料。

当前仍没有 `E1_ready` 或 `E2_ready` 方案，也不授权完整材料优化或二级出水柱实验。唯一直接执行动作改为冻结`B0 + M0`最小判别包：先区分大环内酯与匹配阳离子两亲性药物的生物界面响应，再用同一批PS–DVB基材证明浅层、深层和均匀羧酸分布确实可制造、可表征且不堵孔。两门未通过前不进入C0吸附因果实验。
