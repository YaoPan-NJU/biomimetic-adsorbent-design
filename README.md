...
当前没有通过的主方案或备选方案，`primary_design` 与 `backup_design` 均为 `null`。

Pan Yao 已纠正此前的路线偏移：生物组织分布和蛋白结合证据用于发现、解释并约束人工材料机制，不能自动把蛋白、表达构建体或折叠肽带入最终材料。主方案必须具有至少一个落实到人工材料的硬功能对应；动态对应改为可选，不再是通过门槛。

当前活动候选是 **S1-SYN**：hL-FABP 启发的非蛋白人工 PFOA/长链 PFCA 选择性吸附材料。识别单元仍是尚待冻结的非肽胍基/酰胺—有限低极性链段受体；载体类别不再预设，介孔硅只保留为机制验证候选。目标是在真实二级出水中同时获得可解释的竞争选择性和不被惰性载体质量显著稀释的工程容量。

当前唯一下一动作是按 [`S1_SYNTHETIC_RECOVERY_BRIEF.md`](rounds/synthetic_recovery_1/S1_SYNTHETIC_RECOVERY_BRIEF.md) 提交一个确切非肽受体、匹配对照、材料类型中立的载体/自支撑结构比较及最终选择路线 SOP。在该设计通过前，不订购、不合成、不开展实验，也不继续 S11 Gate 1b/Gate 2。
