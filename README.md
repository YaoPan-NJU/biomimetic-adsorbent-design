...
此前的 **S1-SYN** PFOA 路线暂停但未终止。当前组合首位为 **BPA-1**：ERRγ 静态几何启发、非模板化、非蛋白的刚性双端夹持路线。它在攻击后为 84/100，尚不是主方案。Pan Yao 已授权继续深化 BPA-1；本轮从 `2E2R` 坐标得到 BPA 两酚氧间距约 9.27 Å，并冻结 1,2-di(pyridin-4-yl)ethyne（`R1-BPE`）作为首选可溶机制探针以及删除/距离错配系列，见 [`GEOMETRY_AND_RECEPTOR_TRIAGE.md`](rounds/bpa1_deep_design_1/GEOMETRY_AND_RECEPTOR_TRIAGE.md)。

当前下一动作是攻击 `R1-BPE` 的中性水溶解度、聚集、合成/对照可比性和分析可辨识性。只有可溶几何门仍可信，才设计可聚合衍生物并比较材料架构；在此之前不选择载体、不订购、不合成、不开展实验，也不继续 S11 Gate 1b/Gate 2。
...
4. 已冻结 `R1-BPE` 及单端删除、无氮疏水、短距和长距对照。它只是可溶机制探针；水中无聚集、距离特异增量和真实二级出水保持性未通过前，不进入材料化。
