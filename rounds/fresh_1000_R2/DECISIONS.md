...
## 2026-07-30 BPA R2 广度优先 slot 1 执行（D 组首槽）

### 一、角度地图与预筛（`angle_maps/bpa_ANGLES.yaml`）

1. 锁定 R1：A01=86 通过（DmpR 固定酚位点 + 桥连区空间门 COF）/A02=62/A03=64/A04=52/A07=39 终止 + 23 角度枚举。
2. R2_A01（PXR 定向混杂 + 双齿 avidity 双酚类捕获）A 级；R2_A02（PBP 脱水腔）B 级 deferred；R2_A03（MV1 桥裂解）X 级。
3. **决策**：执行 R2_A01。正交于 A01：A01 类内判别（BPA vs BPF），R2_A01 类去除（容忍桥多样性、双齿 avidity 捕获全双酚类），选择性哲学相反、互补。

### 二、slot R2_S05_A01 裁决

- 轨迹 r1 68（1c/2h）→ r2 ~82（0c/1h）。C1（酚位点水相弱氢键 A02 教训 + avidity 磁度未证）转前置量热 ITC go/no-go + 单齿因果对照消解；残 1 high（原创性受双酚 COF/MIP 平台饱和封顶，Li 2017 ChemComm 367 引）。天花板 82-84，未达 85。verdict revise_with_phase0_prerequisite。

### 三、中间产物

- 新原型 PROTO_R2_004（PXR 定向混杂，库 86→87）；机制映射 MECH_bpa_001；角度地图；知识图谱 +1 原型/+1 机制/+1 材料/+1 边。
